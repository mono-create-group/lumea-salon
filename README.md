...
画像は実装キット（salon-demo-kit）の公式素材です。
ファイル名を変えずに差し替えれば、そのまま反映されます。

| ファイル | 用途 | 元サイズ |
|---|---|---|
| `hero/hero.webp` | ファーストビュー（右に人物・左に余白の横位置） | 1586×992 |
| `concept/interior.webp` | コンセプト | 1448×1086 |
| `services/{esthe,eyelash,nail}.webp` | メニュー3枚 | 各 1448×1086 |
| `gallery/*.webp` | ギャラリー6枚（Instagramにも流用） | 短辺 1086 |
| `staff/staff-01.webp` | スタッフ1人目 | 1122×1402 |
| `staff/staff-{02,03}.webp` | スタッフ2・3人目（低解像度参照の4倍拡大。要差し替え） | 361×451 |
| `petals/petal-01〜04.webp` | 舞う花びら（透過・各2〜3KB に最適化済み） | 220×220 |
| `cta.webp` | 最終CTA（hero.webp の人物部分を切り出し） | 667×992 |
| `../og-image.jpg` | OGP | 1200×630 |
| `../favicon.svg` | ファビコン | — |

**注意:** `staff-02` `staff-03` だけはキットに高解像度が無く、参照サムネイルを
4倍拡大したものです。高解像度が手に入り次第、同名で上書きしてください。
ロゴはキットの `logo.svg` が「Lumée」表記でデザイン画の「Luméa」と食い違うため、
テキストロゴ（`src/components/ui/Logo.tsx`）を採用しています。
